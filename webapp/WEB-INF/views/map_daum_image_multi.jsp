	...
	<script>
			...
			$
					...
					.done(
							function(response) {
								...
								loc_message = '<div style="padding:5px;">'
										+ words_ko + '</div>';
		...
		function printMulti() {
			...
			for (var i = 0; i < multiPositions.length; i++) {
				...
				// 마커 이미지 생성
				var multiImage = new daum.maps.MarkerImage(imageSrc, multiSize,
						multiOption);

				// 마커 생성
				var multiMarker = new daum.maps.Marker({
					map : map,
					position : multiPositions[i].latlng,
					title : multiPositions[i].title,
					image : multiImage
				});
			}
		}
